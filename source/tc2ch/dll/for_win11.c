...
//	CreateMyNotificationFont(heightWin11Notify / 2, widthNotifyIcon / 5);
	...
	//以下の処理はnotification表示に関わらず実行する必要があるので、GetWin11ElementSizeに移動
...
//	if ((typeWin11Taskbar == 0) && (Win11Type < 2))	//レジストリのTaskbarSiの設定はビルド22579未満(Win11Type == 1)でないと無効
	if (typeWin11Taskbar == 0)	//レジストリのTaskbarSiの設定はビルド22579未満(Win11Type == 1)でないと無効
	{
...
void ReturnToOriginalTaskBar(void)
{
	...
	SetWindowPos(hwndTaskBarMain, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE | SWP_NOSENDCHANGING);
	//ShowWindow(hwndTClockBarWin11, SW_HIDE);
...
LRESULT CALLBACK WndProcTClockBar_Win11(HWND tempHwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MBUTTONUP:
		...
		if (b_DebugLog) {
			writeDebugLog_Win10("[for_win11.c][WndProcTClockBar_Win11] xPos =", GET_X_LPARAM(lParam));
			writeDebugLog_Win10("[for_win11.c][WndProcTClockBar_Win11] yPos =", GET_Y_LPARAM(lParam));
		}
...
//		cursorPosY = GET_Y_LPARAM(lParam);			//タスクバー内座標
		cursorPosY = posYTaskbar + GET_Y_LPARAM(lParam);		//絶対座標
	...
	if (message == 71)		//WM_WINDOWPOSCHANGED
	{
		...
		else if (tempWidth == modifiedWidthTaskbar_Win11Type2)
		{
...
//	hbmpIconYesWin11Notify = LoadBitmap(hmod, IDB_BITMAP4);	
	...
	wsprintfW(tempString, L"%d", num_notify);
	if (num_notify < 10) {
		if (!TextOutW(hdcYesWin11Notify, posNotifyText.x, posNotifyText.y - posNotifyIcon.y, tempString, 1) && b_DebugLog) {
			writeDebugLog_Win10("[for_win11.c][DrawWin11Notify] TextOutW failed(tag=1)", 999);
			writeDebugLog_Win10("[for_win11.c][DrawWin11Notify] GetLastError=", (int)GetLastError());
		}
	}
	else {
		if (!TextOutW(hdcYesWin11Notify, posNotifyText.x, posNotifyText.y - posNotifyIcon.y, tempString, 2) && b_DebugLog) {
			writeDebugLog_Win10("[for_win11.c][DrawWin11Notify] TextOutW failed(tag=2)", 999);
			writeDebugLog_Win10("[for_win11.c][DrawWin11Notify] GetLastError=", (int)GetLastError());
		}
	}
...
void DrawWin11Notify(BOOL b_forceUpdate)
{
	...
	else if (bEnableWin11NotifyIcon)
	{
		...
		if (b_DebugLog) {
			static DWORD s_lastSkipLogTick = 0;
			DWORD nowTick = GetTickCount();
			if ((intWin11FocusAssist <= 0) && (intWin11NotificationNumber <= 0) &&
				((s_lastSkipLogTick == 0) || (nowTick - s_lastSkipLogTick >= 5000))) {
				s_lastSkipLogTick = nowTick;
				writeDebugLog_Win10("[for_win11.c][DrawWin11Notify] Skip notify icon draw (focus/notify <= 0).", 999);
				writeDebugLog_Win10("[for_win11.c][DrawWin11Notify] focusAssist =", intWin11FocusAssist);
				writeDebugLog_Win10("[for_win11.c][DrawWin11Notify] notificationNumber =", intWin11NotificationNumber);
			}
		}
		...
		if (intWin11NotificationNumberPrev != intWin11NotificationNumber)
		{
			b_update = TRUE;
...
//			if (fillbackcolor || (Win11Type == 2)) {
	...
	{
		if (b_DebugLog) {
			static DWORD s_lastDisabledLogTick = 0;
			DWORD nowTick = GetTickCount();
			if ((s_lastDisabledLogTick == 0) || (nowTick - s_lastDisabledLogTick >= 5000)) {
				s_lastDisabledLogTick = nowTick;
				writeDebugLog_Win10("[for_win11.c][DrawWin11Notify] Win11 notify icon disabled (bEnableWin11NotifyIcon == FALSE).", 999);
			}
		}
		if (fillbackcolor)
		{
...
void LogCursorPos(void) 
{
	...
	if (GetCursorPos(&tempPoint)) {
		writeDebugLog_Win10("[for_win11.c][LogCursorPos] Cursor posX =", tempPoint.x);
		writeDebugLog_Win10("[for_win11.c][LogCursorPos] Cursor posY =", tempPoint.y);
	}
	else {
		writeDebugLog_Win10("[for_win11.c] Failed to get cursor position.", 999);
	}
}
...
void SetMainClockOnTasktray_Win11(void)
{
	...
	//25H2ではContentBridge系ハンドルが遅延生成/名称差分になることがあるため、毎回再探索する。
	...
	if (!bCanUseWin11Layout) {
		...
		CreateClockDC();
		//degraded時はWin11ハンドル再取得と再配置を継続するため遅延応答を常に起動する。
		SetTimer(hwndClockMain, IDTIMERDLL_DELEYED_RESPONSE, 500, NULL);
		if (b_DebugLog) {
