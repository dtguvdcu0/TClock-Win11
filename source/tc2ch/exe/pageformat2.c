...
void OnInit(HWND hDlg, LPARAM lParam)
{
	...
	if(s[0]) CBAddStringUTF8Compat(hDlg, IDC_AMSYMBOL, s);
	GetLocaleInfoUTF8Compat(ilang, LOCALE_S1159, s2, 10);
	if(s2[0] && strcmp(s, s2) != 0)
		CBAddStringUTF8Compat(hDlg, IDC_AMSYMBOL, s2);
	...
	if(s[0]) CBAddStringUTF8Compat(hDlg, IDC_PMSYMBOL, s);
	GetLocaleInfoUTF8Compat(ilang, LOCALE_S2359, s2, 10);
	if(s2[0] && strcmp(s, s2) != 0)
		CBAddStringUTF8Compat(hDlg, IDC_PMSYMBOL, s2);
