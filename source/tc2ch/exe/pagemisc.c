			...
			case IDC_DELREG:
				{
					...
					r = DelRegAll();
					if (r == 0)
						MyMessageBox(hDlg,MyStringUTF8(IDS_DELREGNGINI), "TClock-Win11",MB_OK,MB_ICONEXCLAMATION);
					else if (r == 1)
						MyMessageBox(hDlg,MyStringUTF8(IDS_DELREGOK), "TClock-Win11",MB_OK,MB_ICONINFORMATION);
					else
						MyMessageBox(hDlg,MyStringUTF8(IDS_DELREGNG),"TClock-Win11",MB_OK,MB_ICONEXCLAMATION);
				}
				break;
				//  readme.txtを開く
			case IDC_README1:
...
void OnStartup(HWND hDlg)
{
	...
	if(MyMessageBox(hDlg, MyStringUTF8(IDS_STARTUPLINK),
